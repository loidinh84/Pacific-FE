...
export default function EditProfileModal({ isOpen, onClose, user, onProfileUpdated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 animate-in fade-in duration-150">
      <div className="relative w-full max-w-lg bg-[#142247]/95 border border-cyan-400/40 rounded-3xl shadow-[0_25px_60px_rgba(0,0,0,0.7),0_0_35px_rgba(6,182,212,0.18)] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/[0.04]">
          <div>
            <h2 className="text-lg font-bold text-white font-heading tracking-tight">Chỉnh sửa hồ sơ cá nhân</h2>
            <p className="text-xs text-cyan-200/70 mt-0.5 font-medium">Cập nhật thông tin tài khoản của bạn</p>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-full border border-white/20 bg-white/10 hover:bg-white/20 text-white transition-all cursor-pointer"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto custom-scrollbar">
          {errorMessage && (
            <div className="p-3 rounded-xl text-xs bg-rose-500/20 border border-rose-500/30 text-rose-200 flex items-center gap-2">
          ...
          {successMessage && (
            <div className="p-3 rounded-xl text-xs bg-emerald-500/20 border border-emerald-500/30 text-emerald-200 flex items-center gap-2">
          ...
          <div className="flex items-center gap-4 py-2">
            <div className="relative group">
              <div className="w-20 h-20 rounded-2xl bg-[#0e1732] border border-cyan-400/30 flex items-center justify-center text-white text-2xl font-bold overflow-hidden shadow-sm">
                {avatarPreview ? (
            ...
            <div className="flex-1">
              ...
              <label
                htmlFor="avatar-file-input"
                className="inline-block px-3 py-1.5 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-xs font-semibold text-white transition-all cursor-pointer"
              >
                Tải ảnh mới
              </label>
            </div>
          ...
          {/* Họ tên */}
          <div>
            <label className="block text-xs font-semibold text-slate-200 mb-1.5">Họ và tên</label>
            <input
              type="text"
              value={fullName}
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="block text-xs font-semibold text-slate-200">Tiểu sử (Bio)</label>
          ...
          {/* Số điện thoại */}
          <div>
            <label className="block text-xs font-semibold text-slate-200 mb-1.5">Số điện thoại</label>
            <input
              type="text"
              value={phoneNumber}
          ...
          {/* Ngày sinh */}
          <div>
            <label className="block text-xs font-semibold text-slate-200 mb-1.5">Ngày sinh</label>
            <input
              type="date"
              value={dateOfBirth}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-blue-600 hover:bg-blue-500 shadow-[0_4px_14px_rgba(37,99,235,0.4)] active:scale-95 transition-all flex items-center gap-2 cursor-pointer disabled:opacity-60"
            >
              {isLoading ? (
                <>
