...
export function SpeciesDetailHero({ species }) {
  ...
  const taxonomy = species.taxonomy || {
    kingdom: language === "en" ? "Animalia" : "Animalia (Động vật)",
    phylum: language === "en" ? "Chordata" : "Chordata (Dây sống)",
    class:
      language === "en"
        ? species.categoryEn || "Chondrichthyes"
        : species.categoryVi || "Chondrichthyes (Cá sụn)",
    order: language === "en" ? "Lamniformes" : "Lamniformes (Cá mập chuột)",
    family: "Lamnidae",
    genus: "Carcharodon",
  ...
  const size = language === "en" ? (species.sizeEn || species.size) : species.size;
  const weight = language === "en" ? (species.weightEn || species.weight) : species.weight;
  const status = language === "en" ? (species.statusEn || species.statusVi) : species.statusVi;
  const audioLabel = language === "en" ? (species.audioLabelEn || "Play sound") : (species.audioLabelVi || "Phát âm thanh thủy âm");
    ...
    <section className="relative pt-24 pb-10 px-4 md:px-8 max-w-6xl mx-auto">
        ...
        <div className="shrink-0">
          {hasSound ? (
            ...
            >
              {isPlayingAudio ? (
                <>
                  ...
                  <span className="text-cyan-300 font-bold">{t("speciesDetail.audioPlaying")}</span>
                ...
                <>
                  ...
                  <span>{t("speciesDetail.audioBtn")}</span>
                </>
              )}
            </button>
          ) : (
            <div className="flex items-center gap-1.5 text-[11px] text-white/35 px-3 py-1.5 rounded-full border border-white/5 bg-white/[0.02]">
              <VolumeX size={13} />
              <span>{t("speciesDetail.noAudio")}</span>
            </div>
          )}
        </div>
      </div>

      {/* ── 2. 3D COVERFLOW GALLERY ── */}
            ...
            {images.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all cursor-pointer ${
                  i === activeImageIdx ? "w-8 bg-cyan-400" : "w-2 bg-white/30 hover:bg-white/60"
                }`}
              />
            ))}
          </div>
      ...
      <div className="flex items-center justify-between my-6 w-full">
        ...
        >
          ...
          <span>{t("speciesDetail.btnView3D")}</span>
        </button>
        ...
        >
          {isFavorited ? (
            <>
              ...
              <span>{t("speciesDetail.btnFavorited")}</span>
            </>
          ) : (
            <>
              <Favorite className="w-4 h-4 text-white/70" />
              <span>{t("speciesDetail.btnFavorite")}</span>
            </>
          )}
        </button>
      </div>
      ...
      <div className="mt-8 pt-8 border-t border-white/10 space-y-6">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-lg md:text-xl font-bold text-white font-heading">
            {t("speciesDetail.profileTitle")}
          </h2>
        </div>

        {/* Bảng phân loại khoa học (Bố cục 4 cột rộng rãi) */}
        <div className="bg-[#0b172a] border border-white/10 rounded-2xl p-5">
          <p className="text-white/60 text-xs font-semibold uppercase tracking-wider mb-4 flex items-center gap-2">
            <Compass size={14} className="text-cyan-400" />
            {t("speciesDetail.taxonomyTitle")}
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
            ...
            <div className="bg-white/5 rounded-xl p-3 border border-white/5 space-y-2">
              <div>
                <span className="text-white/40 text-[11px] block">{t("speciesDetail.kingdom")}</span>
                <span className="font-semibold text-white mt-0.5 block leading-snug">{taxonomy.kingdom}</span>
              </div>
              <div className="pt-2 border-t border-white/5">
                <span className="text-white/40 text-[11px] block">{t("speciesDetail.phylum")}</span>
                <span className="font-semibold text-white mt-0.5 block leading-snug">{taxonomy.phylum}</span>
              </div>
            </div>
            ...
            <div className="bg-white/5 rounded-xl p-3 border border-white/5 space-y-2">
              <div>
                <span className="text-white/40 text-[11px] block">{t("speciesDetail.class")}</span>
                <span className="font-semibold text-cyan-300 mt-0.5 block leading-snug">{taxonomy.class}</span>
              </div>
              <div className="pt-2 border-t border-white/5">
                <span className="text-white/40 text-[11px] block">{t("speciesDetail.order")}</span>
                <span className="font-semibold text-white mt-0.5 block leading-snug">{taxonomy.order}</span>
              </div>
            </div>
            ...
            <div className="bg-white/5 rounded-xl p-3 border border-white/5 space-y-2">
              <div>
                <span className="text-white/40 text-[11px] block">{t("speciesDetail.family")}</span>
                <span className="font-semibold text-white mt-0.5 block leading-snug">{taxonomy.family}</span>
              </div>
              <div className="pt-2 border-t border-white/5">
                <span className="text-white/40 text-[11px] block">{t("speciesDetail.genus")}</span>
                <span className="font-semibold text-cyan-300 italic mt-0.5 block leading-snug">{taxonomy.genus}</span>
              </div>
            </div>

            {/* Loài (Species) */}
            <div className="bg-cyan-500/10 rounded-xl p-3 border border-cyan-500/25 flex flex-col justify-center">
              <span className="text-cyan-400/80 text-[11px] block font-semibold">{t("speciesDetail.species")}</span>
              <span className="font-bold text-cyan-300 text-sm italic mt-1 block leading-snug">
                {taxonomy.species}
              </span>
              <span className="text-white/50 text-[11px] mt-1 block">
                {language === "en" ? species.nameEn : species.nameVi}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-[#0b172a] border border-white/10 rounded-xl p-3.5 flex items-center gap-3">
            ...
            <div>
              <span className="text-white/40 text-[10px] block uppercase font-medium">{t("speciesDetail.maxSize")}</span>
              <span className="text-white font-bold text-xs sm:text-sm">{size}</span>
            </div>
          </div>
          ...
          <div className="bg-[#0b172a] border border-white/10 rounded-xl p-3.5 flex items-center gap-3">
            ...
            <div>
              <span className="text-white/40 text-[10px] block uppercase font-medium">{t("speciesDetail.depthZone")}</span>
              <span className="text-white font-bold text-xs sm:text-sm">{species.depth}</span>
            </div>
          </div>
          ...
          <div className="bg-[#0b172a] border border-white/10 rounded-xl p-3.5 flex items-center gap-3">
            ...
            <div>
              <span className="text-white/40 text-[10px] block uppercase font-medium">{t("speciesDetail.weight")}</span>
              <span className="text-white font-bold text-xs sm:text-sm">{weight || (language === "en" ? "Updating" : "Đang cập nhật")}</span>
            </div>
          </div>
          ...
          <div className="bg-[#0b172a] border border-white/10 rounded-xl p-3.5 flex items-center gap-3">
            ...
            <div>
              <span className="text-white/40 text-[10px] block uppercase font-medium">{t("speciesDetail.conservationStatus")}</span>
              <span className="text-rose-300 font-bold text-xs sm:text-sm">{status}</span>
            </div>
          </div>
        </div>
              ...
              {scientificDossier.map((dossier, idx) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-cyan-400 text-xs font-bold tracking-wide uppercase">
                      {dossier.category}
                    </span>
                  </div>
